    ...
    Writer: fmt::Write,
{
    ...
    fn inspect(&mut self, writer: &mut Writer, info: Self::InfoType, bus: &mut Bus) -> Result<(), Self::Error>;
}

/// Control the execution of a CPU device for debugging purposes
pub trait Debug<Address, Instant, Bus, Writer>: Inspect<Address, Instant, Bus, Writer> + Step<Address, Instant, Bus>
where
    Address: Copy,
    Bus: BusAccess<Address, Instant>,
    Writer: fmt::Write,
...
mod test {
    ...
    use std::str;
    use crate::bus::{self, SimpleBusError, BusAdapter};
    ...
    //impl<T: bus::Error> From<T> for Error {
    //    fn from(err: T) -> Self {
    //        Self::BusError
    //    }
    //}
    ...
    /*
    impl From<SimpleBusError> for Error {
        fn from(err: SimpleBusError) -> Self {
            Self::BusError
        }
    }
    ...
    */
    ...
    impl BusAccess<u32, Instant> for Memory {
        ...
        fn read(&mut self, _now: Instant, addr: u32, data: &mut [u8]) -> Result<usize, Self::Error> {
    ...
    impl BusAccess<u16, Instant> for Output {
        ...
        fn read(&mut self, _now: Instant, addr: u16, data: &mut [u8]) -> Result<usize, Self::Error> {
            Ok(0)
        }

        fn write(&mut self, _now: Instant, addr: u16, data: &[u8]) -> Result<usize, Self::Error> {
    ...
    impl BusAccess<u64, Instant> for FixedBus {
        ...
        fn read(&mut self, now: Instant, addr: u64, data: &mut [u8]) -> Result<usize, Self::Error> {
            if (0..0x1_0000).contains(&addr) {
                self.memory.read(now, addr as u32 % 0x1_0000, data)
                    .map_err(|_| Error::BusError)
            } else {
                self.output.read(now, addr as u16, data)
                    .map_err(|_| Error::BusError)
            }
        }
        ...
        fn write(&mut self, now: Instant, addr: u64, data: &[u8]) -> Result<usize, Self::Error> {
            if (0..0x1_0000).contains(&addr) {
                self.memory.write(now, addr as u32 % 0x1_0000, data)
                    .map_err(|_| Error::BusError)
            } else {
                self.output.write(now, addr as u16, data)
                    .map_err(|_| Error::BusError)
            }
        }
    }
    ...
    impl BusAccess<u64, Instant> for DynamicBus {
        ...
        fn read(&mut self, now: Instant, addr: u64, data: &mut [u8]) -> Result<usize, Self::Error> {
            for (range, device) in self.devices.iter_mut() {
                if range.contains(&addr) {
                    return device.read(now, addr, data)
                        .map_err(|_| Error::BusError);
                }
            }
            Ok(0)
        ...
        fn write(&mut self, now: Instant, addr: u64, data: &[u8]) -> Result<usize, Self::Error> {
            for (range, device) in self.devices.iter_mut() {
                if range.contains(&addr) {
                    return device.write(now, addr, data)
                        .map_err(|_| Error::BusError);
                }
            }
            Ok(0)
    ...
    fn test_static_system() {
        ...
        let location = 0x100;
        bus.memory.write_beu32(Instant::now(), 0x0000, location as u32).unwrap();

        for i in 0..100 {
            bus.memory.write_beu32(Instant::now(), location + 4 * i as u32, 1 + i as u32).unwrap();
        }
    ...
    fn test_dynamic_system() {
        ...
        let mut bus = DynamicBus {
            devices: vec![
                (0..0x1_0000, Box::new(BusAdapter::new(memory, |addr| addr as u32, |_| Error::BusError))),
                (0x2_0000..0x2_0010, Box::new(BusAdapter::new(output, |addr| addr as u16, |_| Error::BusError))),
            ],
        };
        ...
        bus.write_beu32(Instant::now(), 0x0000, location as u32).unwrap();

        for i in 0..100 {
            bus.write_beu32(Instant::now(), location + 4 * i as u64, 1 + i as u32).unwrap();
        }
        ...
        fn run_dynamic_test(mut bus: Bus, cpu: &mut dyn Step<u64, Instant, Bus, Error = Error>) -> Result<(), Error> {
